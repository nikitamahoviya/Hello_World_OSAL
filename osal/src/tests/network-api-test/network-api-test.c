...
void Server_Fn(void)
{
    ...
    OS_SocketAccept(serv_socket_id, &connsock_id, &addr, OS_PEND);
...
void TestStreamNetworkApi(void)
{
    ...
    expected    = OS_SUCCESS;
    actual      = OS_SocketOpen(&serv_socket_id, OS_SocketDomain_INET, OS_SocketType_STREAM);
    UtAssert_True(actual == expected, "OS_SocketOpen() (%ld) == OS_SUCCESS", (long)actual);
    UtAssert_True(OS_ObjectIdDefined(serv_socket_id), "s_socket_id (%lu) != 0", OS_ObjectIdToInteger(serv_socket_id));
    ...
    actual = OS_SocketBind(serv_socket_id, &s_addr);
    UtAssert_True(actual == expected, "OS_SocketBind() (%ld) == OS_SUCCESS", (long)actual);
    ...
    /* OS_SocketAccept */
    expected = OS_INVALID_POINTER;
    actual   = OS_SocketAccept(serv_socket_id, NULL, NULL, 0);
    UtAssert_True(actual == expected, "OS_SocketAccept() (%ld) == OS_INVALID_POINTER", (long)actual);

    expected = OS_INVALID_POINTER;
    actual   = OS_SocketAccept(serv_socket_id, NULL, &temp_addr, 10);
    UtAssert_True(actual == expected, "OS_SocketAccept() (%ld) == OS_INVALID_POINTER", (long)actual);

    expected = OS_INVALID_POINTER;
    actual   = OS_SocketAccept(serv_socket_id, &temp_id, NULL, 10);
    UtAssert_True(actual == expected, "OS_SocketAccept() (%ld) == OS_INVALID_POINTER", (long)actual);
